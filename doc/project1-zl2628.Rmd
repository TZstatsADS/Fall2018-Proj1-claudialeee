...
# What are different people talking about?  
...
vec.f = c()
for (i in word) {
  vec.f = c(vec.f, d.f$freq[d.f$word==i])
}
...
vec.m = c()
for (i in word) {
  vec.m = c(vec.m, d.m$freq[d.m$word==i])
}

# plot
heat.df = data.frame(female=vec.f, male=vec.m)
row.names(heat.df) = word
pheatmap(as.matrix(heat.df))
```
  
From this graph, we can see that male are more likely to talk about first topic while female are more likely to talk about words such as son in second topic.  
